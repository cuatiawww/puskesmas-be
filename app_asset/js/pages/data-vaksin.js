(function ($) {
  ...
  $(document).ready(function () {
    ...
    // =======================
    function initTable() {
      if (!$.fn.DataTable || !$('#table-data-vaksin').length) return;

      if ($.fn.DataTable.isDataTable('#table-data-vaksin')) {
        $('#table-data-vaksin').DataTable().destroy();
      }

      $('#table-data-vaksin').DataTable({
        destroy: true,
        autoWidth: false,
        pageLength: 10,
    ...
    $(document).on('pjax:end', '#pjax-data-vaksin', function () {
      initTable();
    });
    ...
    // =======================
    $(document).on('click', '#table-data-vaksin .btn-delete', function (e) {
      e.preventDefault();
      ...
      const action = $btn.attr('href') || buildApiUrl('data-vaksin/delete');
        ...
        $.post(action, payload, function (resp) {
          if (resp && resp.success) {
            Swal.fire({ icon: 'success', title: 'Berhasil', text: resp.message || 'Data dihapus' })
              .then(function () {
                if ($.pjax) {
                  $.pjax.reload({ container: '#pjax-data-vaksin', timeout: 2000 });
                } else {
