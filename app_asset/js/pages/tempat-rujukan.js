(function ($) {
  ...
  $(document).ready(function () {
    ...
    // =======================
    function initTable() {
      if (!$.fn.DataTable || !$('#table-tempat-rujukan').length) return;

      if ($.fn.DataTable.isDataTable('#table-tempat-rujukan')) {
        $('#table-tempat-rujukan').DataTable().destroy();
      }

      $('#table-tempat-rujukan').DataTable({
        destroy: true,
        autoWidth: false,
        pageLength: 10,
    ...
    $(document).on('pjax:end', '#pjax-tempat-rujukan', function () {
      initTable();
    });
    ...
    // =======================
    $(document).on('click', '#table-tempat-rujukan .btn-delete', function (e) {
      e.preventDefault();
      ...
      const action = $btn.data('url') || buildApiUrl('tempat-rujukan/delete');
        ...
        $.post(action, payload, function (resp) {
          if (resp && resp.success) {
            Swal.fire({ icon: 'success', title: 'Berhasil', text: resp.message || 'Data dihapus' })
              .then(function () {
                if ($.pjax) {
                  $.pjax.reload({ container: '#pjax-tempat-rujukan', timeout: 2000 });
                } else {
