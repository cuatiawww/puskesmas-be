(function ($) {
  ...
  $(document).ready(function () {
    ...
    // =======================
    function initTable() {
      if (!$.fn.DataTable || !$('#table-kategori-rujukan').length) return;

      if ($.fn.DataTable.isDataTable('#table-kategori-rujukan')) {
        $('#table-kategori-rujukan').DataTable().destroy();
      }

      $('#table-kategori-rujukan').DataTable({
        destroy: true,
        autoWidth: false,
        pageLength: 10,
    ...
    $(document).on('pjax:end', '#pjax-kategori-rujukan', function () {
      initTable();
    });
    ...
    // =======================
    $(document).on('click', '#table-kategori-rujukan .btn-delete', function (e) {
      e.preventDefault();
      ...
      const action = $btn.data('url') || buildApiUrl('kategori-rujukan/delete');
        ...
        $.post(action, payload, function (resp) {
          if (resp && resp.success) {
            Swal.fire({ icon: 'success', title: 'Berhasil', text: resp.message || 'Data dihapus' })
              .then(function () {
                if ($.pjax) {
                  $.pjax.reload({ container: '#pjax-kategori-rujukan', timeout: 2000 });
                } else {
