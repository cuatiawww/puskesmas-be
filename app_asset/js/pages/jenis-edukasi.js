(function ($) {
  ...
  $(document).ready(function () {
    ...
    // =======================
    function initTable() {
      if (!$.fn.DataTable || !$('#table-jenis-edukasi').length) return;

      if ($.fn.DataTable.isDataTable('#table-jenis-edukasi')) {
        $('#table-jenis-edukasi').DataTable().destroy();
      }

      $('#table-jenis-edukasi').DataTable({
        destroy: true,
        autoWidth: false,
        pageLength: 10,
    ...
    $(document).on('pjax:end', '#pjax-jenis-edukasi', function () {
      initTable();
    });
    ...
    // =======================
    $(document).on('click', '#table-jenis-edukasi .btn-delete', function (e) {
      e.preventDefault();
      ...
      const action = $btn.data('url') || buildApiUrl('jenis-edukasi/delete');
        ...
        $.post(action, payload, function (resp) {
          if (resp && resp.success) {
            Swal.fire({ icon: 'success', title: 'Berhasil', text: resp.message || 'Data dihapus' })
              .then(function () {
                if ($.pjax) {
                  $.pjax.reload({ container: '#pjax-jenis-edukasi', timeout: 2000 });
                } else {
