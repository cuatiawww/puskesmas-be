(function ($) {
  ...
  $(document).ready(function () {
    ...
    // =======================
    function initTable() {
      if (!$.fn.DataTable || !$('#table-icd-ix').length) return;

      if ($.fn.DataTable.isDataTable('#table-icd-ix')) {
        $('#table-icd-ix').DataTable().destroy();
      }

      $('#table-icd-ix').DataTable({
        destroy: true,
        autoWidth: false,
        pageLength: 10,
    ...
    $(document).on('pjax:end', '#pjax-icd-ix', function () {
      initTable();
    });
    ...
    // =======================
    $(document).on('click', '#table-icd-ix .btn-delete', function (e) {
      e.preventDefault();
      ...
      const action = $btn.data('url') || buildApiUrl('data-icd-ix/delete');
        ...
        $.post(action, payload, function (resp) {
          if (resp && resp.success) {
            Swal.fire({ icon: 'success', title: 'Berhasil', text: resp.message || 'Data dihapus' })
              .then(function () {
                if ($.pjax) {
                  $.pjax.reload({ container: '#pjax-icd-ix', timeout: 2000 });
                } else {
