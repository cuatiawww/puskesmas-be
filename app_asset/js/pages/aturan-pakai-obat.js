(function ($) {
  ...
  $(document).ready(function () {
    ...
    // =======================
    function initTable() {
      if (!$.fn.DataTable || !$('#table-aturan-pakai-obat').length) return;

      if ($.fn.DataTable.isDataTable('#table-aturan-pakai-obat')) {
        $('#table-aturan-pakai-obat').DataTable().destroy();
      }

      $('#table-aturan-pakai-obat').DataTable({
        destroy: true,
        autoWidth: false,
        pageLength: 10,
    ...
    $(document).on('pjax:end', '#pjax-aturan-pakai-obat', function () {
      initTable();
    });
    ...
    // =======================
    $(document).on('click', '#table-aturan-pakai-obat .btn-delete', function (e) {
      e.preventDefault();
      ...
      const action = $btn.data('url') || buildApiUrl('aturan-pakai-obat/delete');
        ...
        $.post(action, payload, function (resp) {
          if (resp && resp.success) {
            Swal.fire({ icon: 'success', title: 'Berhasil', text: resp.message || 'Data dihapus' })
              .then(function () {
                if ($.pjax) {
                  $.pjax.reload({ container: '#pjax-aturan-pakai-obat', timeout: 2000 });
                } else {
