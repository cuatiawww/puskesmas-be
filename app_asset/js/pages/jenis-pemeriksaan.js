(function ($) {
  ...
  $(document).ready(function () {
    ...
    // =======================
    function initTable() {
      if (!$.fn.DataTable || !$('#table-jenis-pemeriksaan').length) return;

      if ($.fn.DataTable.isDataTable('#table-jenis-pemeriksaan')) {
        $('#table-jenis-pemeriksaan').DataTable().destroy();
      }

      $('#table-jenis-pemeriksaan').DataTable({
        destroy: true,
        autoWidth: false,
        pageLength: 10,
    ...
    $(document).on('pjax:end', '#pjax-data-jenis-pemeriksaan', function () {
      initTable();
    });
    ...
    // =======================
    $(document).on('click', '#table-jenis-pemeriksaan .btn-delete', function (e) {
      e.preventDefault();
      ...
      const action = $btn.attr('href') || buildApiUrl('jenis-pemeriksaan/delete');
        ...
        $.post(action, payload, function (resp) {
          if (resp && resp.success) {
            Swal.fire({ icon: 'success', title: 'Berhasil', text: resp.message || 'Data dihapus' })
              .then(function () {
                if ($.pjax) {
                  $.pjax.reload({ container: '#pjax-data-jenis-pemeriksaan', timeout: 2000 });
                } else {
