(function ($) {
  ...
  $(document).ready(function () {
    ...
    // =======================
    function initTable() {
      if (!$.fn.DataTable || !$('#table-tipe-pemeriksaan').length) return;

      if ($.fn.DataTable.isDataTable('#table-tipe-pemeriksaan')) {
        $('#table-tipe-pemeriksaan').DataTable().destroy();
      }

      $('#table-tipe-pemeriksaan').DataTable({
        destroy: true,
        autoWidth: false,
        pageLength: 10,
    ...
    $(document).on('pjax:end', '#pjax-data-tipe-pemeriksaan', function () {
      initTable();
    });
    ...
    // =======================
    $(document).on('click', '#table-tipe-pemeriksaan .btn-delete', function (e) {
      e.preventDefault();
      ...
      const action = $btn.attr('href') || buildApiUrl('tipe-pemeriksaan/delete');
        ...
        $.post(action, payload, function (resp) {
          if (resp && resp.success) {
            Swal.fire({ icon: 'success', title: 'Berhasil', text: resp.message || 'Data dihapus' })
              .then(function () {
                if ($.pjax) {
                  $.pjax.reload({ container: '#pjax-data-tipe-pemeriksaan', timeout: 2000 });
                } else {
